...
class DRM(nn.Module):
    ...
    def forward(self, low_disp, left_img, right_img):
        ...
        scale_factor = left_img.size(-1) / low_disp.size(-1) # 计算视差图与左图像之间的缩放因子
        if scale_factor == 1.0:
            disp = low_disp
        else:
            disp = F.interpolate(low_disp, size=left_img.size()[-2:], mode='bilinear', align_corners=False) # 对 low_disp 进行双线性插值，使其分辨率与左图像相同。
            disp = disp * scale_factor

        # Warp right image to left view with current disparity
        warped_right = reconstruct(right_img, disp)[0]  # [B, C, H, W]
