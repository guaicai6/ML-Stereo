...
from core.ml_stereo import MLStereo, autocast
...
def validate_middlebury(model, iters=32, split='H', mixed_prec=False):
    ...
    for val_id in range(len(val_dataset)):
        ...
        out = (epe_flattened > 2.0)  # 计算误差大于 2.0 的像素比例。
        image_out = out[val].float().mean().item()
        image_epe = epe_flattened[val].mean().item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default="checkpoints/20240723.pth")
    parser.add_argument('--dataset', help="dataset for evaluation", required=True,
                        choices=["eth3d", "kitti", "things"] + [f"middlebury_{s}" for s in 'FHQ'],default='middlebury_H')
    ...
    if args.restore_ckpt is not None:
        ...
        checkpoint = torch.load(args.restore_ckpt,map_location='cuda:0')
